    ...
    public class DatalinkRun
    {
        ...
        public DatalinkRun(TransformSettings transformSettings, ILogger logger, long parentAuditKey, DexihDatalink datalink, DexihHub hub, InputColumn[] inputColumns, TransformWriterOptions transformWriterOptions)
        {
            ...
            if (primaryTarget == null)
            {
                ...
                {
                    TransformWriterMethod  = TransformWriterTarget.ETransformWriterMethod.None
                };   
            }
            else
            {
                WriterTarget = NewTransformWriterTarget(primaryTarget, datalinkTable, auditConnection);
            }
            ...
            foreach (var target in Datalink.DexihDatalinkTargets.Where(c=>c.NodeDatalinkColumnKey != null))
            {
                ...
                var childWriterTarget = NewTransformWriterTarget(target, datalinkTable, auditConnection);
                WriterTarget.Add(childWriterTarget, nodePath);
            }
        }

        private TransformWriterTarget NewTransformWriterTarget(DexihDatalinkTarget target, DexihDatalinkTable datalinkTable, Connection auditConnection)
        {
            ...
            var writerTarget = new TransformWriterTarget(targetConnection, targetTable, Datalink.LoadStrategy, writerResult, _transformWriterOptions, targetConnection, rejectTable);
            return writerTarget;
        }
...
//                await WriterTarget.Initialize(cancellationToken);
                ...
                WriterTarget.OnProgressUpdate += Datalink_OnProgressUpdate;
                WriterTarget.OnStatusUpdate += Datalink_OnStatusUpdate;
        ...
        public Task Run(CancellationToken cancellationToken)
        {
            try
            {
                ...
                return WriterTarget.WriteRecordsAsync(Reader.sourceTransform, Datalink.UpdateStrategy,cancellationToken);
            }
