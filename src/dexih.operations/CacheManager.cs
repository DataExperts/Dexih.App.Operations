...
namespace dexih.operations
{
    ...
    public class CacheManager
    {
	    ...
	    public async Task LoadDashboardDependencies(DexihDashboard dashboard, DexihRepositoryContext dbContext)
	    {
		    ...
		    await dbContext.Entry(dashboard).Collection(a => a.DexihDashboardItems).Query().Include(c => c.Parameters).Where(c => c.IsValid && dashboard.Key == c.DashboardKey).LoadAsync();
...
//                        }
                        ...
                        if (item.CustomFunctionKey != null)
                        {
                            ...
                            {
                                customFunction = await dbContext.DexihCustomFunctions
	                                .Include(c=>c.DexihCustomFunctionParameters)
	                                .SingleOrDefaultAsync(c => c.Key == item.CustomFunctionKey && c.IsValid);
...
//			}
                ...
                foreach (var item in datalinkTransform.DexihDatalinkTransformItems)
                {
					...
					if (item.CustomFunctionKey != null)
					{
						...
						if (customFunction == null)
						{
							...
							if (customFunction == null)
							{
								throw new CacheManagerException($"The custom function with the key {item.CustomFunctionKey} was not found.");
							}
        ...
        public async Task LoadColumnValidationDependencies(DexihColumnValidation columnValidation, DexihRepositoryContext dbContext)
        {
            if(columnValidation.LookupColumnKey != null)
            {
                ...
                if (column == null)
                {
	                throw new CacheManagerException($"The load column validation dependencies failed as the column with the key {columnValidation.LookupColumnKey} was not found.");
                }
        ...
        public async Task AddConnections(IEnumerable<long> connectionKeys, bool includeTables, DexihRepositoryContext dbContext)
        {
            foreach (var connectionKey in connectionKeys)
            {
                ...
                if(connection == null)
                {
                    ...
                    if (connection == null)
                    {
	                    throw new CacheManagerException($"The connection with the key {connectionKey} was not found.");
                    }
	    ...
	    public void AddFileFormats(IEnumerable<long> fileFormatKeys, DexihHub hub)
	    {
		    foreach (var fileFormatKey in fileFormatKeys)
		    {
			    ...
			    if(fileFormat == null)
			    {
				    ...
				    if (fileFormat == null)
				    {
					    throw new CacheManagerException($"The file format with the key {fileFormatKey} was not found.");
				    }
...
//		    }
        ...
        public async Task AddDatajobs(IEnumerable<long> datajobKeys, DexihRepositoryContext dbContext)
        {
            foreach (var datajobKey in datajobKeys)
            {
                ...
                if(existingJob == null)
                {
                    ...
                    if (job == null)
                    {
	                    throw new CacheManagerException($"The job with the key {datajobKey} was not found.");
                    }
	    ...
	    public async Task AddColumnValidations(IEnumerable<long> columnValidationKeys, DexihRepositoryContext dbContext)
	    {
		    foreach (var columnValidationKey in columnValidationKeys)
		    {
			    ...
			    if(columnValidation == null)
			    {
				    ...
				    if (columnValidation == null)
				    {
					    throw new CacheManagerException($"The column validation with the key {columnValidationKey} was not found.");
				    }
        ...
        public async Task AddCustomFunctions(IEnumerable<long> customFunctionKeys, DexihRepositoryContext dbContext)
        {
            foreach (var customFunctionKey in customFunctionKeys)
            {
                ...
                if(customFunction == null)
                {
                    ...
                    if (customFunction == null)
                    {
	                    throw new CacheManagerException($"The custom function with the key {customFunctionKey} was not found.");
                    }
	    ...
	    public void AddViews(IEnumerable<long> viewKeys, DexihHub hub)
	    {
		    foreach (var viewKey in viewKeys)
		    {
			    ...
			    if(view == null)
			    {
				    ...
				    if (view == null)
				    {
					    throw new CacheManagerException($"The view with the key {viewKey} was not found.");
				    }
	    ...
	    public void AddDashboards(IEnumerable<long> dashboardKeys, DexihHub hub)
	    {
		    foreach (var dashboardKey in dashboardKeys)
		    {
			    ...
			    if(dashboard == null)
			    {
				    ...
				    if (dashboard == null)
				    {
					    throw new CacheManagerException($"The dashboard with the key {dashboardKey} was not found.");
				    }
	    ...
	    public async Task AddListOfValues(IEnumerable<long> listOfValuesKeys, DexihRepositoryContext dbContext)
	    {
		    foreach (var key in listOfValuesKeys)
		    {
			    ...
			    if(listOfValues == null)
			    {
				    ...
				    if (listOfValues == null)
				    {
					    throw new CacheManagerException($"The list of values with the key {key} was not found.");
				    }
	    ...
	    public void AddApis(IEnumerable<long> apiKeys, DexihHub hub)
	    {
		    foreach (var apiKey in apiKeys)
		    {
			    ...
			    if(api == null)
			    {
				    ...
				    if (api == null)
				    {
					    throw new CacheManagerException($"The api with the key {apiKey} was not found.");
				    }
	    ...
	    public async Task<DexihDatalinkTest> GetDatalinkTest(long datalinkTestKey, DexihRepositoryContext dbContext)
	    {
		    ...
		    if (datalinkTest == null)
		    {
			    throw new CacheManagerException($"The datalink test with the key {datalinkTestKey} was not found.");
		    }
