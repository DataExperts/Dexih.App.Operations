...
//        }
//
//        public async Task SendDatalinkData(DexihHub hub, DexihDatalink datalink, InputColumn[] inputColumns, TransformWriterOptions transformWriterOptions, BufferBlock<object> buffer, CancellationToken cancellationToken)
//        {
//            try
//            {
//                var transformOperations = new TransformsManager(_transformSettings);
//                var runPlan = transformOperations.CreateRunPlan(hub, datalink, inputColumns, null, null, transformWriterOptions);
...
//                while ((await transform.ReadAsync(cancellationToken)) && (transformWriterOptions.SelectQuery == null || totalCount < transformWriterOptions.SelectQuery.Rows || transformWriterOptions.SelectQuery.Rows == -1) &&
